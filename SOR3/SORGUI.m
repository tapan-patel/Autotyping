...
% Last Modified by GUIDE v2.5 18-Aug-2014 11:41:00
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
choice = questdlg('Draw free-hand or rectangle?','Left half of the box ROI','Rectangle','Free-hand','Cancel','Rectangle');
try
idx = handles.curr_idx;
catch
    errordlg('You must first select a video from the listbox.','Bad input','modal');
...
%         I = rgb2gray(I);
        ...
        if(obj_idx~=0)
            ...
            hold on
            plot(B{1}(:,2),B{1}(:,1));
            handles.Info(idx).ROIs.BWobject5 = Lobj==obj_idx;
            pause(2);
        else
            try
                 mask=false(size(info.ref_frame,1),size(info.ref_frame,2));
            ...
            B = bwboundaries(bw);
            plot(B{1}(:,2),B{1}(:,1));
            handles.Info(idx).ROIs.BWobject5 = bw;
            pause(2);
            end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
choice = questdlg('Draw free-hand or rectangle?','Left surface of the box ROI','Rectangle','Free-hand','Cancel','Rectangle');
try
idx = handles.curr_idx;
catch
    errordlg('You must first select a video from the listbox.','Bad input','modal');
...
%         I = rgb2gray(I);
        ...
        if(obj_idx~=0)
            ...
            hold on
            plot(B{1}(:,2),B{1}(:,1));
            handles.Info(idx).ROIs.BWobject6 = Lobj==obj_idx;
            pause(2);
        else
            try
                 mask=false(size(info.ref_frame,1),size(info.ref_frame,2));
            ...
            B = bwboundaries(bw);
            plot(B{1}(:,2),B{1}(:,1));
            handles.Info(idx).ROIs.BWobject6 = bw;
            pause(2);
            end
...
handles.Info(handles.curr_idx).Novel(1) = val;
guidata(hObject,handles);
...
val = get(hObject,'Value');
handles.Info(handles.curr_idx).Novel(2) = val;
guidata(hObject,handles);
...
val = get(hObject,'Value');
handles.Info(handles.curr_idx).Novel(3) = val;
guidata(hObject,handles);
...
val = get(hObject,'Value');
handles.Info(handles.curr_idx).Novel(4) = val;
guidata(hObject,handles);
...
val = get(hObject,'Value');
handles.Info(handles.curr_idx).Novel(5) = val;
guidata(hObject,handles);
...
val = get(hObject,'Value');
handles.Info(handles.curr_idx).Novel(6) = val;
guidata(hObject,handles);
