function varargout = ZeroMazeGUI(varargin)
warning off
%ZeroMazeGUI M-file for ZeroMazeGUI.fig
%      ZeroMazeGUI, by itself, creates a new ZeroMazeGUI or raises the existing
%      singleton*.
%
%      H = ZeroMazeGUI returns the handle to a new ZeroMazeGUI or the handle to
%      the existing singleton*.
%
%      ZeroMazeGUI('Property','Value',...) creates a new ZeroMazeGUI using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to ZeroMazeGUI_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      ZeroMazeGUI('CALLBACK') and ZeroMazeGUI('CALLBACK',hObject,...) call the
%      local function named CALLBACK in ZeroMazeGUI.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help ZeroMazeGUI

% Last Modified by GUIDE v2.5 17-Aug-2014 16:15:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ZeroMazeGUI_OpeningFcn, ...
    'gui_OutputFcn',  @ZeroMazeGUI_OutputFcn, ...
    'gui_LayoutFcn',  [], ...
    'gui_Callback',   []);
...
% --- Executes just before ZeroMazeGUI is made visible.
function ZeroMazeGUI_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for ZeroMazeGUI
...
% UIWAIT makes ZeroMazeGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ZeroMazeGUI_OutputFcn(hObject, eventdata, handles)
...
if(strcmp(selection,'open'))
    ...
    if(idx>numel(handles.Info))
        ...
        if(isfield(handles.Info(idx),'diameter'))
            set(handles.edit16,'String',num2str(handles.Info(idx).diameter));
        end
    end
end
...
try
    ...
    B = bwboundaries(info.ROIs.Open1);
    for k = 1:length(B)
        boundary = B{k};
    ...
    B = bwboundaries(info.ROIs.Open2);
    for k = 1:length(B)
        boundary = B{k};
        plot(hax,boundary(:,2), boundary(:,1), 'b', 'LineWidth', 2);
    end
    ...
    B = bwboundaries(info.ROIs.Closed1);
    for k = 1:length(B)
        boundary = B{k};
        plot(hax,boundary(:,2), boundary(:,1), 'b', 'LineWidth', 2);
    end
    
    B = bwboundaries(info.ROIs.Closed2);
    for k = 1:length(B)
        boundary = B{k};
        plot(hax,boundary(:,2), boundary(:,1), 'b', 'LineWidth', 2);
    end
...
if(exist(savefile))
    ...
    switch choice
        case 'Yes'
            save(savefile,'Info');
            disp(['Zero maze Initializations saved to ' savefile]);
    end
else
    save(savefile,'Info');
    disp(['Zero maze Initializations saved to ' savefile]);
...
switch button
    case 'Now'
        ...
        ZeroMazeBatch(handles.folder_name);
...
%     errordlg('You must enter a time for empty box. Enter 0:0 if unknown.','Bad Input','modal')
%     set(handles.pushbutton6,'Enable','on');
%     uicontrol(hObject)
%     return
% end
...
multiWaitbar('Determining frame #',0,'Name',handles.curr_file);
multiWaitbar('Determining background image',0);
...
% If the user did not enter an empty box time, we have to estimate the
% background image. 
frames = mmcount(vidname);
start_frame = TimeToFrame(vidname,1,frames,info.start_time);
multiWaitbar('Determining frame #',.5);
if(~isfield(info,'ref_time') || info.ref_time==0)
    handles.Info(idx).ref_frame = EstimateBackground(vidname,frames,start_frame);
    handles.Info(idx).ref_idx = 1;
    ref_frame = 1;
    multiWaitbar('Determining background image',1,'Color','g');
    set(handles.edit7,'String','n/a');
else
    ref_frame = TimeToFrame(vidname,1,frames,info.ref_time);
    V = mmread(vidname,ref_frame);
    handles.Info(idx).ref_frame = rgb2gray(V(end).frames.cdata);
    handles.Info(idx).ref_idx = ref_frame;
end
multiWaitbar('Determining frame #',1,'Color','g');
...
BW = hfree.createMask;
handles.Info(idx).ROIs.Open1=BW;
% Delete figure
delete(hfig);
...
handles.Info(idx).ROIs.Closed1 = hobject1.createMask;
...
hfree = imfreehand(hax);
BW = hfree.createMask;
handles.Info(idx).ROIs.Open2 = BW;
% Delete figure
delete(hfig);
...
hax = imgca;
hobject1 = imfreehand(hax);
handles.Info(idx).ROIs.Closed2 = hobject1.createMask;
...
h=figure;
hax = gca;
imshow(info.start_frame,'Parent',hax);

h1 = msgbox('Draw an ellipse around the inner portion of the maze. Double-click when done to proceed.');
uiwait(h1);
h_ell = imellipse(hax);
wait(h_ell);
BW_inner = h_ell.createMask;
h1 = msgbox('Draw an ellipse around the outer portion of the maze. Double-click when done');
uiwait(h1);
h_ell = imellipse(hax);
wait(h_ell);
BW_outer = h_ell.createMask;
Maze = BW_outer - BW_inner;
Maze(Maze<0) = 0;
handles.Info(idx).ROIs.Maze = Maze;
handles.Info(idx).ROIs.inner_maze = BW_inner;
delete(h);
...
if(isfield(handles.Info(idx),'thresh') && ~isempty(handles.Info(idx).thresh))
    ...
    set(handles.text22,'String',['Threshold = ' num2str(thresh)]);
end
if(handles.Info(idx).ref_time==0)
...
set(handles.text22,'String',['Threshold = ' num2str(val)]);
try
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
ZeroMaze(handles.Info(handles.curr_idx),'DISPLAY',1);
...
handles.Info(handles.curr_idx).diameter = str2double(get(hObject,'String'));
guidata(hObject,handles);
% --- Executes during object creation, after setting all properties.
