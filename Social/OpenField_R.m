function [TimeSitting, TimeCorners, TimeOuter, TimeCenter, TimeInner,path_length, thigmotaxis,Sitting_in_periphery,TimeSitting_in_corner,Motion] = OpenField_R(COM,start_idx,frames,Info,fps,Tail,duration,box_dim)
...
TimeCorners=0; TimeOuter=0; TimeCenter=0; TimeInner=0;

%% First figure out conversion from pixels to inches: box is 12x15 inch
periphery = 1.0; % 2 inches from the walls is the periphery
% figure
...
center_BW_L = imerode(Info.ROIs.surface_right,ones(ceil(4.5*periphery*px_per_inch_L))); 
inner_BW_L = imerode(Info.ROIs.surface_right,ones(ceil(2*periphery*px_per_inch_L)))-center_BW_L;
outer_BW_L = Info.ROIs.surface_right - inner_BW_L-center_BW_L;
% Corners: 1 - top right, 2 - top right, 3 - bottom right, 4 - bottom right
ul = zeros(4,2);
...
for i=1:4
    Corners_L(i).BW = false(size(Info.ROIs.surface_right));
...
Location = single(zeros(frames,1));
...
% Videos should be either 10 or 30 mins long

idx = floor(linspace(start_idx,frames,duration/(60*5)+1));

for i=1:length(idx)-1
    TimeCorners(1,i) = nnz(Location(idx(i):idx(i+1))==1)./fps;
    TimeOuter(1,i) = nnz(Location(idx(i):idx(i+1))==2)./fps;
    TimeInner(1,i) = nnz(Location(idx(i):idx(i+1))==3)./fps;
    TimeCenter(1,i) = nnz(Location(idx(i):idx(i+1))==4)./fps;
end
% 
...
for i=1:length(idx)-1
    TimeSitting(1,i) = nnz(evnts(idx(i):idx(i+1))<thr)./fps;
end

%% Sitting in a corner - use tail coordinates to determine if mouse is in corner
...
for i=1:length(idx)-1
    TimeSitting_in_corner(1,i) = nnz(Sitting_in_corner(idx(i):idx(i+1)))/fps;
end

Motion = Motion/px_per_inch_L;

%% Eliminate interactions that occur within 1 inch of periphery
periphery = 1; % 2 inches from the walls is the periphery
% figure
% imshow(Info.ref_frame); axis image;
% hold on
C = regionprops(Info.ROIs.surface_right,'BoundingBox');
...
Sitting_in_periphery = Location==1 | Location== 2; %(evnts<thr & (Location==1 | Location==2));
