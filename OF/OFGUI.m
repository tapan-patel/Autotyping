function varargout = OFGUI(varargin)
% OFGUI MATLAB code for OFGUI.fig
%      OFGUI, by itself, creates a new OFGUI or raises the existing
%      singleton*.
%
%      H = OFGUI returns the handle to a new OFGUI or the handle to
%      the existing singleton*.
%
%      OFGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in OFGUI.M with the given input arguments.
%
%      OFGUI('Property','Value',...) creates a new OFGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OFGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OFGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OFGUI

% Last Modified by GUIDE v2.5 25-Aug-2014 14:33:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @OFGUI_OpeningFcn, ...
    'gui_OutputFcn',  @OFGUI_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before OFGUI is made visible.
function OFGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OFGUI (see VARARGIN)

% Choose default command line output for OFGUI
...
% UIWAIT makes OFGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = OFGUI_OutputFcn(hObject, eventdata, handles)
...
if(strcmp(selection,'normal'))
    ...
    if(isfield(handles.Info(idx),'duration'))
        set(handles.edit3,'String',handles.Info(idx).duration_str);
    end
...
if(exist(savefile))
    ...
    switch choice
        case 'Yes'
            save(savefile,'Info');
            disp(['OF Initializations saved to ' savefile]);
    end
else
    save(savefile,'Info');
    disp(['OF Initializations saved to ' savefile]);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
h = waitbar(.1,'Please wait while determining background. This may take several minutes');
set(hObject,'Enable','off');
set(hObject,'Selected','on');
...
for i=1:numel(files)
    ...
    waitbar(i/numel(files),h,'Please wait while determining background. This may take several minutes');
end
set(hObject,'Enable','on');
set(hObject,'Selected','off');
...
try
    ...
    if(Mouse)
        ...
        if(isfield(info.ROIs,'surface') && ~isempty(info.ROIs.surface))
            ...
            px_per_inch = mean([C.BoundingBox(3)/handles.Info(idx).dimensions_L C.BoundingBox(4)/handles.Info(idx).dimensions_W]);
            
            inner_BW = imerode(info.ROIs.surface,ones(ceil(2*info.perimeter*px_per_inch)));
...
choice = questdlg('Draw free-hand or rectangle?','Surface of the box ROI','Rectangle','Free-hand','Cancel','Rectangle');
try
idx = handles.curr_idx;
catch
    errordlg('You must first select a video from the listbox.','Bad input','modal');
...
if(~isfield(handles.Info(idx),'ref_frame') || isempty(handles.Info(idx).ref_frame))
    h = msgbox('Please wait. Estimating background image');
...
OF(handles.Info(handles.curr_idx),'DISPLAY',1);
